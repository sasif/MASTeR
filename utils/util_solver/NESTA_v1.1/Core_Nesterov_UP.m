...
for k = 0:maxiter-1,
   ...
   %---- Updating yk 
    ...
    if ~mod(k+1,Verbose )
        ...
        %         global T_frames ROW COL
        %         Ir_cube = [];
        %         for frame = 2:T_frames-1;
        %             Ir_cube(:,:,frame) = reshape(xk((frame-1)*ROW*COL+1:frame*ROW*COL),ROW,COL);
        %             figure(333); imagesc(abs(Ir_cube(:,:,frame)));
        %             axis image; colormap gray; pause(1/60);
        %         end
